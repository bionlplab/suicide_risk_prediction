...
class TemporalProcessor:
    ...
    def load_and_prepare_train(self, df: pd.DataFrame) -> pd.DataFrame:
        """Cast labels, parse time, sort, and add original_index."""
    ...
    def compute_temporal_features_from_times(self, timestamps: List[pd.Timestamp]) -> Dict[str, List[float]]:
        """Compute 13 per-step features; pad to 5 timestamps."""
        ...
        while len(ts) < 5:
            ts.append(ts[-1])
        # minimal but stable placeholders
        for bi in range(5):
    ...
    def transform_temporal_features(self, features: Dict[str, List[float]]) -> np.ndarray:
        """Flatten to 65-d in FEATURE_KEYS_13 order and scale."""
        row = []
        for name in FEATURE_KEYS_13:
            vals = features[name]
            if len(vals) < 5:
...
class BinDataset(Dataset):
    """One sample = a 5-post bin; ONLY text embeddings + temporal features (no SDOH)."""
    def __init__(self, bin_indices: List[np.ndarray], df: pd.DataFrame,
                 processor: TemporalProcessor, embeddings: np.ndarray,
    ...
    def _build(self):
        ...
        print(f"Preparing data for {len(self.bin_indices)} bins...")
        for i, bin_idx in enumerate(self.bin_indices):
            ...
            for _, row in bin_rows.head(5).iterrows():
                ...
                seq_embeddings.append(rfeat.astype(np.float32))

            while len(seq_embeddings) < 5:
                seq_embeddings.append(
                    seq_embeddings[-1].copy() if seq_embeddings
                    else np.zeros(self.emb_dim, dtype=np.float32)
                )
            seq_embeddings = seq_embeddings[:5]

            data.append({
                'user_id': i,
                'label': label,
                'seq_embeddings': np.array(seq_embeddings, dtype=np.float32),  # [5, emb_dim]
                'temporal_features': temporal_features,
                'time_gaps': np.array([float(j*24) for j in range(5)], dtype=np.float32),  # not used
            })
    ...
    def __getitem__(self, idx):
        item = self.items[idx]
        scaled_temporal = self.processor.transform_temporal_features(item['temporal_features'])
        return {
            'user_id': item['user_id'],
            'label': torch.tensor(item['label'], dtype=torch.long),
            'seq_embeddings': torch.tensor(item['seq_embeddings'], dtype=torch.float32),
...
class TestBinDataset(Dataset):
    """Test-time bins built from grouped indices; ONLY embeddings + temporal features."""
    def __init__(self, grouped_indices: List[np.ndarray], df: pd.DataFrame,
                 proc: TemporalProcessor, emb_dim: int,
    ...
    def __getitem__(self, i):
        ...
        if self.per_post_emb is not None:
            rbt = self.per_post_emb[rows['original_index'].values].astype(np.float32)  # [5, emb_dim]
        else:
            rbt = self.per_bin_emb[i].astype(np.float32)  # [5, emb_dim]
        ...
        return {
            ...
            'seq_embeddings': torch.tensor(rbt, dtype=torch.float32),
...
def collate(batch):
    return {
        ...
        'seq_embeddings': torch.stack([b['seq_embeddings'] for b in batch]),         # [B, 5, emb_dim]
        'temporal_features': torch.stack([b['temporal_features'] for b in batch]),   # [B, 5, 13]
        'time_gaps': torch.stack([b['time_gaps'] for b in batch]),                   # [B, 5]
    }
...
class LSTMTemporalModel(nn.Module):
    def __init__(self, emb_dim=768, temporal_dim=13,
        ...
        # Project per-step text embedding
        self.text_proj = nn.Sequential(
            nn.Linear(emb_dim, hidden_dim),
            nn.GELU(), nn.Dropout(dropout), nn.LayerNorm(hidden_dim)
        )

        # Encode global temporal block (5x13 -> 32), use LayerNorm to avoid BN small-batch issues
        ...
        # Fuse per-step text with global temporal embedding
    ...
    def forward(self, seq_embeddings, temporal_features, time_gaps=None):
        # seq_embeddings: [B, 5, emb_dim]
        B, n_bins = seq_embeddings.shape[:2]

        text_proj = self.text_proj(seq_embeddings.view(-1, self.emb_dim)).view(B, n_bins, -1)  # [B,5,H]

        temporal_flat = temporal_features.view(B, -1)               # [B, 65]
        temporal_emb  = self.temporal_encoder(temporal_flat)        # [B, 32]
        temporal_exp  = temporal_emb.unsqueeze(1).expand(-1, n_bins, -1)  # [B,5,32]

        fused_bins = torch.cat([text_proj, temporal_exp], dim=-1)   # [B,5,H+32]
        fused_bins = self.bin_fusion(fused_bins)                    # [B,5,H]

        lstm_out, _ = self.temporal_lstm(fused_bins)                # [B,5,H]
        mask = torch.ones(B, n_bins, dtype=torch.bool, device=lstm_out.device)
        user_embed = self.time_attention(lstm_out, mask)            # [B,H]
        logits = self.classifier(user_embed)                        # [B,C]
...
def train_lstm(train_loader, val_loader, device, emb_dim, num_epochs=80, lr=8e-5, weight_decay=1e-3, patience=10):
    model = LSTMTemporalModel(emb_dim=emb_dim, hidden_dim=384, lstm_layers=2,
    ...
    for ep in range(1, num_epochs+1):
        ...
        for batch in train_loader:
            opt.zero_grad()
            out = model(batch['seq_embeddings'].to(device),
...
def main():
    ...
    proc = TemporalProcessor(max_posts_per_bin=5, n_bins=5)
    ...
    for spec in ENSEMBLE_SPECS:
        ...
        train_ds = BinDataset(train_bins, dfp, proc, train_emb, emb_dim=EMB_DIM, is_training=True)
        val_ds   = BinDataset(val_bins,   dfp, proc, train_emb, emb_dim=EMB_DIM, is_training=False)
    ...
    if len(test0_bins) > 0:
        ...
        for spec in trained_specs:
            ...
            test0_ds = BinDataset(test0_bins, dfp, proc, train_emb_full, emb_dim=EMB_DIM, is_training=False)
            test0_loader = DataLoader(test0_ds, batch_size=128, shuffle=False, collate_fn=collate)

            model = LSTMTemporalModel(emb_dim=EMB_DIM, hidden_dim=384, lstm_layers=2, dropout=0.2, num_classes=4).to(device)
    ...
    for spec in trained_specs:
        ...
        test_proc = TemporalProcessor(max_posts_per_bin=5, n_bins=5)
    ...
    np.save("output.npy", preds_final.astype(np.int64))
